...
internal class UnsignedCodersTests {
    ...
    private fun <T> testEncodePairs(serializer: KSerializer<T>, vararg pairs: Pair<String, T>) {
        pairs.forEach { (expectedResult, value) ->
            assertEquals(expectedResult, MsgPack(inlineEncoders = MsgPackUnsignedSupport.ALL_ENCODERS, inlineDecoders = MsgPackUnsignedSupport.ALL_DECODERS).encodeToByteArray(serializer, value).toHex())
        }
    }
    private fun <T> testDecodePairs(serializer: KSerializer<T>, vararg pairs: Pair<String, T>) {
        pairs.forEach { (value, expectedResult) ->
            assertEquals(expectedResult, MsgPack(inlineEncoders = MsgPackUnsignedSupport.ALL_ENCODERS, inlineDecoders = MsgPackUnsignedSupport.ALL_DECODERS).decodeFromByteArray(serializer, value.hexStringToByteArray()))
        }
    }
}
